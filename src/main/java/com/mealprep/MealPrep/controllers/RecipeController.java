...
public class RecipeController {
    ...
    public RecipeIngredientsDTO example() {
        ...
        RecipeSeasonalityDTO example3 = new RecipeSeasonalityDTO("example",
                                                                 exampleSeasonSet);
    ...
    public ResponseEntity<Recipe> registerRecipe(@RequestHeader(value = "Authorization") String token, @RequestBody Recipe newRecipe) {
    ...
    @PatchMapping(value = "/ingredients", consumes =
            {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity<Optional<Recipe>> updateIngredients(@RequestBody RecipeIngredientsDTO params) {
            Optional<Recipe> result =
                    recipeService.updateIngredients(params.getRecipeName(),
    ...
    @PatchMapping(value = "/seasonality", consumes =
            {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity<Optional<Recipe>> updateSeasonality(@RequestBody RecipeSeasonalityDTO params) {
        Optional<Recipe> result =
                recipeService.updateSeasonality(params.getRecipeName(),
    ...
    @PatchMapping(value = "/type", consumes =
            {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity<Optional<Recipe>> updateMealType(
            @RequestBody RecipeMealTypeDTO params) {

            String recipeName = params.getRecipeName();
            Optional<Recipe> result = recipeService.updateMealType(recipeName,
                                                           params.getMealType());
        if (result.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NO_CONTENT).body(result);
        } else {

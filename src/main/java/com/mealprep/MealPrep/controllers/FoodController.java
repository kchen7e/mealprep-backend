...
import com.mealprep.MealPrep.entities.api.food.FoodDTO;
...
//                foodService.registerFood(params.getFoodName()));
    ...
    @PatchMapping(value = "/foundas", consumes =
    ...
    public ResponseEntity<Object> updateCommercialForms(
        ...
        return ResponseEntity.status(HttpStatus.OK).body(
                foodService.updateCommercialForm(params.getFoodName(),
                                                 params.getFoundAs()));
    }
    ...
    public ResponseEntity<Object> example() {
        ...
        CommercialForm cf = new CommercialForm( CommercialForm.SaleUnitType.CAN,
                                               1, "Woolworths");
        cfs.add(cf);
        return ResponseEntity.status(HttpStatus.OK).body(
                foodService.updateCommercialForm("example", cfs));
    }
}
